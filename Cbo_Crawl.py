...
def Cbo_Crawl(movie_name,movie_id, movie_year):
    ...
    for i in range(1,urls.__len__()+1,1):
        ...
        if name == movie_name and year == str(movie_year):
            ...
            print(url)
            Get_data(url,movie_name, movie_id, movie_year)
            break
        else:
            print("continue")


def Get_data(url,movie_name,movie_id, movie_year):
    print("fuction used")
    driver.get(url)
    time.sleep(1)
    ...
    for i in range(1,market.__len__()+1,1):
        ...
        print(market_name)
        print(data_week)
        Store(market_name,data_week,movie_name,movie_id,movie_year)

def Store(market_name,data_week,movie_name,movie_id,movie_year):
    print("------store-----")
    with open("./data/" + str(movie_year) + "/" + str(movie_id) + ".txt","a+") as w:
        w.writelines(movie_name + '\n')
...
if __name__ == '__main__':
    ...
    for year in range(2007,2019):
        ...
        with open("./票房网/movie_" + str(year) + ".csv", "r") as r:
            first = 1
            lines = r.readlines()
            for line in lines:
                if first == 1:
                ...
                try:
                    Cbo_Crawl(movie_name, movie_id, year)
                    if not os.path.exists("./data/" + str(year) + "/" + str(movie_id) + ".txt"):
                        print("----- No data -----")
